...
export class TicketsService {
  ...
  async create(createTicketDto: CreateTicketDto) {
    const { cantidad_boletos: numberTickets, sorteo_id: lotteryId, price } = createTicketDto
    
    if (numberTickets > 100000) {
      throw new HttpException('The maximum number of tickets that can be created is 100,000', HttpStatus.BAD_REQUEST);
    }
    ...
    try {
      ...
      throw new HttpException('An error occurred while creating the ticket', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async find(findTicketDto: findTicketDto) {
    ...
    try {
      if (!lotteryId) {
        throw new HttpException('The lottery id is required', HttpStatus.BAD_REQUEST);
      }
  
      const ticktes = await this.ticketRepository.find({where: {lottery_id: lotteryId}})

      if (!ticktes) {
        throw new HttpException('No tickets found for this lottery', HttpStatus.NOT_FOUND);
      }

      return {message: 'tickets found successfully', data: ticktes}
    } catch (error) {
      console.error('Error finding tickets', error);
      throw new HttpException('An error occurred while finding the ticket', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async findTicketEndingWith(findTicketEndingDto: findTicketEndingDto) {
    ...
    if (!number || isNaN(Number(number))) {
      throw new HttpException('The number is required and should be number', HttpStatus.BAD_REQUEST);
    }
    ...
    try {
      ...
      if (!tickets) {
        throw new HttpException('No tickets found for this lottery and number', HttpStatus.NOT_FOUND);
      }
  ...
  async random(findTicketRandomDto: findTicketRandomDto) {
    ...
    if (!quantity || isNaN(Number(quantity))) {
      throw new HttpException('The quantity is required and should be number', HttpStatus.BAD_REQUEST);
    }

    try {
      const tickes = await this.ticketRepository.createQueryBuilder('ticket')
       .where('ticket.lottery_id = :lotteryId', { lotteryId })
  ...
  async update(updateTicketDto: UpdateTicketDto) {
    ...
    if (!status || !['disponible', 'pagado', 'proceso'].includes(status)){
      throw new HttpException('The status is required and should be disponible, pagado or proceso', HttpStatus.BAD_REQUEST);
    }

    try {
      const ticket = await this.ticketRepository.findOneBy({ lottery_id: lotteryId, number })
      ...
      ticket.owner = owner;
      ticket.expiration = new Date(DateTime.local().plus({ minutes: expiretionMinuts }).toISO());
      ...
      throw new HttpException('An error occurred while updating the ticket', HttpStatus.INTERNAL_SERVER_ERROR);
  ...
  async reset(findTicketDto: findTicketDto){
    ...
    const updatedTickets = await this.ticketRepository.createQueryBuilder('ticket')
      .update(Ticket)
  ...
  async delete(findTicketDto: findTicketDto){
    ...
    const deletedTickets = await this.ticketRepository.createQueryBuilder('ticket')
      .delete()
      .where('ticket.lottery_id = :lotteryId', { lotteryId })
      .execute()
